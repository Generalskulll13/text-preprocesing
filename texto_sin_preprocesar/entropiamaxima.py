...
def main():
    text_data_headers = []
    archivo1=open("texto_sin_preprocesar/listadepalabras.txt",'r')
    archivo2=open("texto_sin_preprocesar/resultados.txt",'w')
    for linea in archivo1.readlines():
        text_data_headers.append(tokenize(linea)[0])

    

    text_data = pd.read_csv('texto_sin_preprocesar/dataset.csv', names=text_data_headers)
